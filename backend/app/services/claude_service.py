...
- **Provider and team names:** When the transcript mentions a provider or team that is close to an option in the provided lists — similar sounding, phonetically alike, or a clear misspelling (e.g. "Doctor Gahar" → "Dr. Ghajar") — select the matching option. Do not use the transcribed text verbatim when a correct option exists. For provider_name: match to the closest provider in the enum. For call_teams: use the exact team titles from the schema (first and last name for each doctor). Infer the best team(s) from provider, context, and available teams. In the summary, use the correct provider name from the schema and the doctor team names as listed, not the raw transcript.
