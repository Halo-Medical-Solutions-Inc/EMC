...
- **Provider and team names:** When the transcript mentions a provider or team that is close to an option in the provided lists — similar sounding, phonetically alike, or a clear misspelling (e.g. "Doctor Slash" → "Ron Sklash") — select the matching option. Do not use the transcribed text verbatim when a correct option exists. For provider_name: match to the closest provider in the enum. For call_teams: infer the best team(s) based on provider, context, and available teams. In the summary, always use the correct provider/team names from the schema, not the raw transcript.
